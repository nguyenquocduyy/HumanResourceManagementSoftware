...
{
    public partial class RListRewardDate : DevExpress.XtraReports.UI.XtraReport
    {
        public RListRewardDate()
        {
            InitializeComponent();
        }
        List<T_KhenThuong> ls_KhenThuong;
        public RListRewardDate(List<T_KhenThuong> khenthuong)
        {
            InitializeComponent();
            this.ls_KhenThuong = khenthuong;
            this.DataSource = ls_KhenThuong;
            xrLThang.Text = (ls_KhenThuong[0].createAt).Value.Month.ToString();
            xrLNam.Text = (ls_KhenThuong[0].createAt).Value.Year.ToString();
            loadData();
        }
